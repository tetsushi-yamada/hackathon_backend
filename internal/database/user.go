...
func (repo *UserDatabase) SearchUsersTx(tx *sql.Tx, userName string) ([]*user.User, error) {
	var users []*user.User
	query := `SELECT user_id, user_name, user_description, is_private, created_at, updated_at FROM users WHERE user_name LIKE ?`
	...
	for rows.Next() {
		...
		var createdAt, updatedAt []byte
		var UserDescription sql.NullString
		err = rows.Scan(&user.UserID, &user.UserName, &UserDescription, &user.IsPrivate, &createdAt, &updatedAt)
		if err != nil {
			return nil, err
		}
		if UserDescription.Valid {
